...
import {
  apiGetOpensearchSetupInfo,
  apiRequestReIndexing,
  apiTriggerIndexing,
} from "../../utils/apiUrls";
...
const loadOpensearchSetupInfo = () => {
  return axiosInstance
    .get(apiGetOpensearchSetupInfo)
    .then((resp) => resp.data)
    .then((data) => {
      return data;
    });
...
const Homepage = () => {
  ...
  const displayLabels = {
    connected: "Connected",
    opensearchCertificate: "Certificate",
    opensearchHost: "Opensearch host",
    opensearchIndexAlias: "Opensearch index Alias name",
    opensearchUserName: "Opensearch username",
    indexingCronSchedule: "Indexing cron schedule",
    initialized: "Opensearch configuration loaded",
  };

  const reloadOpensearchSetupInfo = ({ showNotification }) => {
    setIsInProgress(true);
    loadOpensearchSetupInfo()
      .then(setSetupInfo)
      .then(() => {
        if (showNotification)
          toggleNotification({
            type: "success",
            message: "Opensearch setup information reloaded.",
            timeout: 5000,
          });
      })
  ...
  const requestFullSiteReindexing = () => {
    setIsInProgress(true);
    return axiosInstance
      .get(apiRequestReIndexing)
      .then(() => {
        toggleNotification({
          type: "success",
          message: "Rebuilding the index is triggered.",
          timeout: 5000,
        });
      })
      .catch(() => {
  ...
  const triggerIndexingRun = () => {
    setIsInProgress(true);
    return axiosInstance
      .get(apiTriggerIndexing)
      .then(() => {
        toggleNotification({
          type: "success",
          message: "The indexing job to process the pending tasks is started.",
          timeout: 5000,
        });
      })
      .catch(() => {
  ...
  useEffect(() => {
    reloadOpensearchSetupInfo({ showNotification: false });
  }, []);

  if (setupInfo === null) return <LoadingIndicatorPage />;
  else
    return (
