...
const exportContentConfig = () => {
  return axiosInstance
    ...
    .then((response) => {
      ...
      link.setAttribute(
        "download",
        "strapi-plugin-opensearch-contentconfig.json"
      );
...
const loadContentConfig = () => {
  return axiosInstance.get(apiGetContentConfig).then((resp) => resp.data);
};
...
const Configure = () => {
      ...
      importContentConfig(conf)
        .then(() => {
          toggleNotification({
            type: "success",
            message:
              "Collections configuration imported. Please refresh this view.",
            timeout: 5000,
          });
        })
        .catch((err) => {
          toggleNotification({
            type: "warning",
            message:
              "Importing collections configuration failed. An error was encountered.",
            timeout: 5000,
          });
          console.log(err);
        })
    ...
    exportContentConfig()
      ...
      .catch((err) => {
        toggleNotification({
          type: "warning",
          message:
            "Exporting collections configuration failed. An error was encountered.",
          timeout: 5000,
        });
        console.log(err);
      })
  ...
  if (config === null) return <LoadingIndicatorPage />;
  else {
    return (
      <>
            ...
            {displayImportModal && (
              ...
              >
                ...
                <ModalFooter
                  startActions={
                    <Button
                      onClick={() => setDisplayImportModal(false)}
                      variant="tertiary"
                    >
                      Cancel
                    </Button>
