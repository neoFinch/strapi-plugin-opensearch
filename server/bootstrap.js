...
module.exports = async ({ strapi }) => {
    ...
    const configureIndexingService = strapi.plugins['opensearch'].services.configureIndexing;
    ...
    try
    {
      ...
      if (!Object.keys(pluginConfig).includes('indexingCronSchedule'))
        console.warn("The plugin strapi-plugin-opensearch is enabled but the indexingCronSchedule is not configured.");
      else if (!Object.keys(pluginConfig).includes('searchConnector'))
        console.warn("The plugin strapi-plugin-opensearch is enabled but the searchConnector is not configured.");
      else
      {
        const connector = pluginConfig['searchConnector'];
        await osInterface.initializeSearchEngine({host : connector.host, uname: connector.username, 
      ...
      console.error('An error was encountered while initializing the strapi-plugin-opensearch plugin.')
      console.error(err);
    }  
};
