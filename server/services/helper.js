      ...
      for (let s = 0; s < config.length; s++) {
        ...
        if (Object.keys(extractItem).includes("__component")) {
          if (
            ...
            if (
              Object.keys(conf).includes("transform") &&
              conf["transform"] === "markdown"
            )
              val = transformServiceProvider.transform({
                content: val,
                from: "markdown",
              });
...
module.exports = ({ strapi }) => ({
  async getOpensearchInfo() {
    ...
    let data = await osInterface.checkOSConnection();
    console.log({data})
    console.log('pluginConfig : ', pluginConfig)
    console.log('pluginConfig.searchConnector && pluginConfig.searchConnector.host : ', pluginConfig.searchConnector && pluginConfig.searchConnector.host)
    const connected =
      pluginConfig.searchConnector && pluginConfig.searchConnector.host
        ? await osInterface.checkOSConnection()
        : false;
  ...
  async getCurrentIndexName() {
    ...
    let indexName = "strapi-plugin-opensearch-index_000001";
    if (settings) {
  ...
  async getIncrementedIndexName() {
    ...
    return (
      "strapi-plugin-opensearch-index_" + String(number + 1).padStart(6, "0")
    );
  },
  async storeCurrentIndexName(indexName) {
    ...
    if (settings) {
      ...
      await pluginStore.set({
        key: "configsettings",
        value: JSON.stringify(objSettings),
      });
    } else {
    ...
    for (let k = 0; k < fti.length; k++) {
      ...
      if (fieldConfig.index) {
        ...
        if (Object.keys(fieldConfig).includes("subfields")) {
          val = extractSubfieldData({
            config: fieldConfig["subfields"],
            data: data[fti[k]],
          });
