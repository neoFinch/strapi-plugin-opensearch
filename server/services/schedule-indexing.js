...
module.exports = ({ strapi }) => ({
  async addFullSiteIndexingTask () {
      const data = await strapi.entityService.create('plugin::opensearch.opensearch-task', {
          data : {
              collection_name: '',
  ...
  async getItemsPendingToBeIndexed(){
      const entries = await strapi.entityService.findMany('plugin::opensearch.opensearch-task', {
          filters: { indexing_status : 'to-be-done'},
       });
       return entries;     
  },
  async markIndexingTaskComplete (recId) {
      const entries = await strapi.entityService.update('plugin::opensearch.opensearch-task', recId, {
          data : {
