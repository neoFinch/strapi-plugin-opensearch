...
module.exports = ({ strapi }) => ({
  async initializeStrapiOpensearch() {
    await this.cacheConfig();
  },
  async markInitialized() {
    if (!strapi.opensearch)
      strapi.opensearch = {}
    strapi.opensearch.initialized = true;
  },
  isInitialized() {
    return strapi.opensearch?.initialized || false
  },
  async cacheConfig() {
    if (!strapi.opensearch)
      strapi.opensearch = {}
    strapi.opensearch.collectionsconfig = await this.getCollectionsConfiguredForIndexing();
    strapi.opensearch.collections = await this.getCollectionsConfiguredForIndexing();  
  },
